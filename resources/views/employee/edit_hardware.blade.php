<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Hardware Details') }}
        </h2>
    </x-slot>

    <form method="POST" action="{{ route('employee.hardwares.update', $hardware) }}"
        ...
        <div class="mb-5 mx-5">
            ...
            <select name="company_id" id="company_id"
                ...
                @foreach($companies as $company)
                    <option value="{{ $company->id }}" {{ old('company_id', $hardware->company_id) == $company->id ? 'selected' : '' }}>
                        {{ $company->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-5 mx-5">
            ...
            <input type="text" name="warranty" id="warranty"
                   value="{{ old('warranty', $hardware->warranty) }}"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
                   required>
        ...
        <div class="mb-5 mx-5">
            ...
            <input type="text" name="brand" id="brand"
                   value="{{ old('brand', $hardware->brand) }}"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
                   required>
        ...
        <div class="mb-5 mx-5">
            ...
            <textarea 
                ...
                rows="6"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
            >{{ old('description', $hardware->description) }}</textarea>
