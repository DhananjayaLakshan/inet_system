<x-app-layout>
    ...
    <div class="mx-5 my-5" x-data="{ tab: 'weekly' }">
        ...
        <div x-show="tab === 'weekly'" class="mt-6">
            <div class="md:mx-auto bg-white p-4 rounded shadow md:max-w-[81%]">
                ...
                {{-- <div class="mt-6 bg-red-50 mb-10 p-4 rounded shadow">
                    <h3 class="text-lg font-semibold text-red-600 mb-3">Employees who didn't submit weekly expenses</h3>
                    @forelse($companiesNotVisited as $company)
                        <p class="text-gray-800">- {{ $company->name }}</p>
                    @empty
                        <p class="text-gray-500">All employees have submitted their weekly expenses</p>
                    @endforelse
                </div> --}}
                    ...
                    <tbody>
                        @forelse($payments as $payment)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 border">{{ $payment->company->name }}</td>
                                ...
                                <td class="px-4 py-2 border">{{ $payment->description }}</td>
                                ...
                                <form 
                                action="{{ route('employee.payments.destroy', $payment) }}" 
                                method="POST"
        ...
        {{-- All Visits Tab --}}
        <div x-show="tab === 'paymenyts'" class="mt-6">
            {{-- Desktop table --}}
            {{-- <div class="hidden md:block overflow-x-auto">
                <table class="min-w-full border bg-white border-gray-200">
                    <thead class="bg-orange-100">
